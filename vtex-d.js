...
// init CLI
program.version(pkg.version).description("vtex-d CLI for VTEX utils");
...
program
  ...
  .option(
    "--site <site>",
    'Set the site for upload on portal ("default" if not provide)'
  )
...
program
  .command("deploy")
  .description(
    "Auth user and deploy all files (CSS, JS, HTML Templates and HTML SubTemplates) on VTEX"
  )
  .option("--account <account>", "Set the VTEX account name")
  .option("--email <email>", "Set the email account")
  .option(
  ...
  .option(
    "--pathFiles <path>",
    'Set the location to upload assets: "files" or "arquivos"'
  )
...
program.command("dirname").action(() => {
